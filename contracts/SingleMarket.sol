...
contract SingleMarket is EIP712, BasicMetaTransaction {
    ...
    function BuyCustodial2Custodial(HeftyVerseBuyer memory buyer,HeftyVerseSeller memory seller, Voucher.SFTvoucher memory _voucher,Voucher.NFTvoucher memory _voucherNFT, bool isNFT) internal {
        if(!_voucher.toMint && !_voucherNFT.toMint) {
            ...
            uint royaltyAmount;
            if(isNFT) {
                require(INFTTemplate(seller.nftAddress).ownerOf(seller.tokenID) == seller.owner);
                (receiver, royaltyAmount) = INFTTemplate(seller.nftAddress).royaltyInfo(seller.tokenID,buyer.pricePaid);
                token.transferFrom(treasury, receiver , royaltyAmount);

            } else {
                require(ISFTTemplate(seller.nftAddress).balanceOf(seller.owner, seller.tokenID) >= seller.amount);
                (receiver, royaltyAmount) = ISFTTemplate(seller.nftAddress).royaltyInfo(seller.tokenID,buyer.pricePaid);
                token.transferFrom(treasury, receiver , royaltyAmount);
            }
