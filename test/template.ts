...
import { 
  ...
  HeftyVerseMarketplace1155,
  HeftyVerseMarketplace1155__factory} from "../typechain"
...
beforeEach(async() =>{
    ...
    await factory.initialize(NFT.address, template1155.address,Marketplace.address ,marketplace1155.address);
});
...
//   const Tseries = await factory.connect(owner).userLastNFTContracts(owner.address);
...
it("Redeem Voucher: Using marketplace",async () => {
  ...
  await Marketplace.BuyFixedPrice(Voucherbuy, VoucherSell, voucher);
...
it("redeem : royalty keeper default admin",async () => {
  //console.log(owner.address,"owner from test as signer");
  //console.log(superOwner.address,"superowner from test as signer");
  const lazyMinting = new LazyMinting({_contract:NFT, _signer:owner});
  const voucher= await lazyMinting.createVoucher(NFT.address,1,expandTo18Decimals(2),"testURI","0x0000000000000000000000000000000000000000",100);
  //console.log(voucher,"voucher"); 
  await NFT.connect(signers[3]).redeem(voucher,signers[3].address);
});
...
describe("Advanced marketplace secondary buy", async () => {
  it("custodial to custodial buy", async () => {
    ...
    const Tseries = await factory
      .connect(owner)
      .userLastNFTContracts(owner.address);
    const TRS = await new Template__factory(owner).attach(Tseries);
    const addressOfTemplate = Tseries;
    //creating vouchers
    const lazyMinting = new LazyMinting({ _contract: TRS, _signer: owner });
    ...
    await Marketplace.BuyFixedPrice(Voucherbuy2, VoucherSell2, voucher);
  ...
  it("noncustodial to noncustodial buy", async () => {
    //create collection
    await factory
      .connect(owner)
      .create721Token(
  ...
  it("ERROR: Wrong nft Addresses in buyer and seller vouchers", async () => {
    ...
    await usdt
      ...
      .approve(Marketplace.address, expandTo6Decimals(1000));
    await expect(
      Marketplace.BuyFixedPrice(Voucherbuy, VoucherSell, voucher)
    ).to.be.revertedWith("Addresses doesn't match");
  });
  ...
  it("ERROR: Signature of seller not matching at secondary buy(C2C)", async () => {
    ...
    await expect( Marketplace.BuyFixedPrice(Voucherbuy2, VoucherSell2, voucher)).to.be.revertedWith('invalid Seller');
  ...
  it("ERROR: Signature of buyer not matching at secondary buy(C2C)", async () => {
    ...
    await expect( Marketplace.BuyFixedPrice(Voucherbuy2, VoucherSell2, voucher)).to.be.revertedWith('invalid buyer');
...
describe("Marketplace1155 secondary buy",async()=>{
  ...
  it("ERROR custodial to custodial primary buy : buyer and nft mismatched addresses", async () => {
    ...
    await usdt
      .connect(owner)
      .approve(marketplace1155.address, expandTo6Decimals(1000));
    await expect(marketplace1155.Buy(Voucherbuy, VoucherSell, voucher)).to.be.revertedWith("Addresses doesn't match");
    });
    ...
    it("ERROR custodial to custodial primary buy : invalid price", async () => {
      ...
      await usdt
        .connect(owner)
        .approve(marketplace1155.address, expandTo6Decimals(1000));
      await expect(marketplace1155.Buy(Voucherbuy, VoucherSell, voucher)).to.be.revertedWith("price doesn't match");
      });  
//to do
  ...
  it("ERROR custodial to noncustodial primary buy:  mismatched addresses", async () => {
    ...
    await usdt
      .connect(signers[6])
      .approve(marketplace1155.address, expandTo6Decimals(1000));
    await expect (marketplace1155.Buy(Voucherbuy, VoucherSell, voucher)).to.be.revertedWith("mismatched addresses");
  });
...
// });
  ...
  it("ERROR 1155 Buy: Addresses doesn't match",async()=>{
    ...
    it("1155buy: counters mismatched",async()=>{
      ...
      await usdt.connect(owner).approve(marketplace1155.address,expandTo6Decimals(1000));
      //console.log("NFT balance of first buyer before buy:",await TRS.balanceOf(signers[6].address,1));
      await expect(marketplace1155.Buy(Voucherbuy, VoucherSell, voucher)).to.be.revertedWith("counters mismatched");
      })
      ...
      it("ERROR 1155 buy: mismatched amounts",async()=>{
        ...
        await usdt.connect(owner).approve(marketplace1155.address,expandTo6Decimals(1000));
        //console.log("NFT balance of first buyer before buy:",await TRS.balanceOf(signers[6].address,1));
        await expect(marketplace1155.Buy(Voucherbuy, VoucherSell, voucher)).to.be.revertedWith("mismatched amounts");
      })    
...
describe("template1155 testing", async () => {
  ...
  await TRS.connect(signers[4]).redeem(voucher,signers[4].address);
